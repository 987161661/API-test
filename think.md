...
- **变更**:
    ...
    - **一致性修复**: 修复了 AI 导演给不同模型设定不同群名的 Bug。现在所有模型都会收到基于剧本主题生成的统一群名（如“孤岛求生讨论组”）。

### 4. 部署与维护优化
- **Git集成**: 在主页侧边栏增加了“🔄 更新项目”按钮，一键执行 `git pull`，方便非技术用户更新。
- **时间同步**: 优化了聊天界面时间戳，剧本模式下显示虚拟时间而非现实时间。
- **跳转通知**: 上帝模式时间跳转时，现在会自动插入系统提示消息。
...
4.  **一致性保证**: 必须确保 AI 导演生成的设定在所有演员之间是自洽的（Shared Grounding）。我们通过在 Prompt 注入阶段强制统一 Context Name 来解决此问题。
